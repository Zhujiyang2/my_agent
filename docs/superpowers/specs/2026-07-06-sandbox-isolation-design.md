...
- 构造 bwrap 命令：
  ...
  bwrap \
    --ro-bind / / \
    --tmpfs /tmp \
    --dev /dev \                                  # 透传 /dev/davinci* 等 NPU 设备
    ...
    -- <command>
  ```
- Ascend NPU 关键设备文件：`/dev/davinci*`（芯片）、`/dev/davinci_manager`、`/dev/hisi_hdc`、`/dev/vdavinci`。`--dev /dev` 将这些设备完整透传进沙箱。
...
1. **昇腾 NPU 设备透传**：bwrap `--dev /dev` 能否正常透传 `/dev/davinci*`、`/dev/davinci_manager`、`/dev/hisi_hdc`、`/dev/vdavinci`；Docker 容器内 CANN（Ascend Computing Architecture）是否正常工作，`npu-smi` 是否能识别芯片
2. **Ascend Docker Runtime**：bwrap namespace 内 `docker run --runtime=ascend`（或相应的 device plugin 方式）是否正常启动昇腾容器
3. **Docker socket 权限**：bwrap namespace 内能否正常与 Docker daemon 通信
4. **bwrap + Docker -v 路径映射**：宿主机路径通过 bwrap 的 `--bind` 挂载后，Docker `-v` 指定的宿主机路径是否正确解析
5. **网络模型下载**：bwrap `--share-net` 下 `huggingface-cli download`、`wget` 等是否正常工作
6. **bwrap 可用性**：目标 Linux 发行版（openEuler、Kylin、UOS 等昇腾常见系统，以及 Ubuntu/CentOS）是否内置 bwrap
7. **嵌套 bwrap**：Docker 容器内的进程是否会再被 bwrap 包装（不应发生——Docker 内命令由容器内 Shell 执行，不经 agent 的 sandbox-manager）
8. **性能**：bwrap 对 I/O 密集型操作（下载权重、读写模型文件）的性能影响；NPU 计算任务是否受影响
