...
My Agent 是一个终端交互式 AI 编程助手，主要运行在 Linux 环境（昇腾 NPU 服务器）中。核心任务场景是**模型训推**——包括拉取镜像/权重、启动容器（Ascend Docker Runtime）、运行训推命令（vllm-ascend、mindie 等）、做模型评测。Agent 在大量无人值守 / 自动化场景下运行。
...
│  宿主机 /                    → 只读 (--ro-bind)           │
│  宿主机 /tmp                 → tmpfs 可读写                │
│  宿主机 /dev                 → bind 透传 (昇腾 NPU 设备)     │
│  宿主机 /dev/shm             → bind 透传 (CANN 共享内存)    │
...
2. **创建**：sandbox-manager 在宿主机上 `mkdir -p <path>`（bwrap `--bind` 要求源路径存在）
3. **注册**：路径加入 writable 列表
4. **重建**：bwrap 实例重建，新增 `--bind <path> <path>`，使该路径可读写
...
2. Sandbox-manager 校验路径合法性
3. Sandbox-manager 在宿主机上 `mkdir -p /mnt/nvme0/my-agent-workspace`（bwrap `--bind` 要求源路径存在）
4. 路径加入 writable 列表，bwrap 实例重建
...
- 构造 bwrap 命令：
  ...
  bwrap \
    --ro-bind / / \
    --tmpfs /tmp \
    --bind /dev /dev \                            # 透传宿主机全部设备（含 davinci*）
    --bind /dev/shm /dev/shm \                    # CANN/PyTorch 共享内存
    ...
    -- <command>
  ```
- **为什么用 `--bind /dev /dev` 而非 `--dev /dev`**：`--dev /dev` 创建的是全新的最小化 `/dev`（仅 null、zero、random 等基础节点），**不会透传宿主机的 NPU 设备**。必须用 `--bind` 将宿主机 `/dev` 完整挂入沙箱，NPU 设备（`/dev/davinci*`、`/dev/davinci_manager`、`/dev/hisi_hdc`、`/dev/vdavinci`）才能被访问。宿主机 `/dev` 对非 root 用户本来就是只读的，无额外安全风险。
- **`/dev/shm` 透传**：CANN 和 PyTorch 依赖共享内存做进程间通信，如果缺了这个路径训推任务会直接失败。
...
1. **昇腾 NPU 设备透传**：bwrap `--bind /dev /dev` 能否正常透传 `/dev/davinci*`、`/dev/davinci_manager`、`/dev/hisi_hdc`、`/dev/vdavinci`；沙箱内 `npu-smi info` 是否正常识别芯片
2. **Ascend Docker Runtime**：沙箱内 `docker run --runtime=ascend`（或 device plugin 方式）是否正常启动昇腾容器，容器内 CANN 是否正常工作
3. **共享内存**：`--bind /dev/shm /dev/shm` 是否满足 CANN/PyTorch 的共享内存需求，多卡训推任务是否正常
4. **Docker socket 权限**：bwrap namespace 内能否正常与 Docker daemon 通信
5. **Ascend 驱动服务通信**：昇腾 HDC（Host Device Communication）驱动服务是否依赖额外的 Unix socket 或管道，沙箱是否阻断了这些通信路径
6. **bwrap + Docker -v 路径映射**：宿主机路径通过 bwrap 的 `--bind` 挂载后，Docker `-v` 指定的宿主机路径是否正确解析
7. **网络模型下载**：bwrap `--share-net` 下 `huggingface-cli download`、`wget` 等是否正常工作
8. **bwrap 可用性**：目标 Linux 发行版（openEuler、Kylin、UOS 等昇腾常见系统，以及 Ubuntu/CentOS）是否内置或可安装 bwrap
9. **嵌套 bwrap**：Docker 容器内的进程是否会再被 bwrap 包装（不应发生——Docker 内命令由容器内 Shell 执行，不经 agent 的 sandbox-manager）
10. **性能**：bwrap 对 I/O 密集型操作（下载权重、读写模型文件）的性能影响；NPU 计算任务是否受影响
